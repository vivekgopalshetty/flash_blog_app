...
from app import app, db, bycrypt,mail
...
def register():
    ...
    if form.validate_on_submit():
        hashed_pwd=bycrypt.generate_password_hash(form.password.data).decode('utf-8')
...
def send_reset_email(user):
    ...
    if did not make this request ignore this mail and no changes will be made
    '''
    mail.send(msg)
...
def reset_token(token):
    ...
    if user:
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash('your account password has been upadated', 'success')
            return redirect(url_for('login'))
        return redirect('reset_request.html',title="Request Reset",form=form)
    else:
        flash('request reset again','warning')
        form=Requestresetform()
